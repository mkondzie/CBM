...
void write_parameters(float z_alpha, int n_neigh_ch, float sus_perc,
  ...
  ofstream my_file;
  my_file.open("conn_check_parameters.txt", std::ios::out | std::ios::app);
  ...
  for (int i = 0; i < int(file_names.size()); i++) {
    ...
    for (int ch = 0; ch < 128; ch++) {
      ...
      if (ch % 2 == 0) {
        ...
        if (ch_hits[ch] > neigh_med_even + thr_even) {
        ...
        if (ch_hits[ch] < thr_even) {
          ...
          std::cout << "ch: " << ch
                    << " is suspicious, weighted av (sus_even): "
...

          {
            std::cout << "std::abs(h_weighted_av->GetBinContent(ch) - "
                      ...
                      << std::abs(h_weighted_av->GetBinContent(ch + 1) -
                                  h_med_weighted_av->GetBinContent(ch + 1))
                      << "  3.0 * sus_neigh_sigma "
                      << n_sigmas * sus_neigh_sigma << std::endl;
        ...
        if (ch_hits[ch] > neigh_med_odd + thr_odd) {
          // if the channel is extra noisy it's also masked
          noisy_channels.push_back(ch);
        }
        ...
        if (ch_hits[ch] < thr_odd) {
          ...
          std::cout << "__________" << std::endl;
          std::cout << "ch: " << ch
                    << " is suspicious, calculate_weighted_av_ch : "
                    << calculate_weighted_av_ch(ch, h2hits_amp) << std::endl;
          std::cout << "calculate_median of weighted av (sus_odd): "
                    << calculate_median(sus_odd, sus_odd.size()) << std::endl;
          std::cout << "sus_neigh_sigma: " << sus_neigh_sigma << std::endl;

          std::cout << "h_weighted_av->GetBinContent(ch): "
                    << h_weighted_av->GetBinContent(ch + 1) << std::endl;
          std::cout << "h_med_weighted_av->GetBinContent(ch) "
                    << h_med_weighted_av->GetBinContent(ch + 1) << std::endl;
          ...
          if (std::abs(h_weighted_av->GetBinContent(ch + 1) -
            ...
            std::cout << "std::abs(calculate_weighted_av_ch(ch, h2hits_amp) - "
                         "calculate_median(sus_odd, sus_odd.size())) "
                      << std::abs(calculate_weighted_av_ch(ch, h2hits_amp) -
                                  calculate_median(sus_odd, sus_odd.size()))
                      << "  3.0 * sus_neigh_sigma "
                      << n_sigmas * sus_neigh_sigma << std::endl;
            ///////////////////////////////////
            std::cout << "std::abs(h_weighted_av->GetBinContent(ch) - "
                         "h_med_weighted_av->GetBinContent(ch)) "
                      << std::abs(h_weighted_av->GetBinContent(ch + 1) -
                                  h_med_weighted_av->GetBinContent(ch + 1))
                      << "  3.0 * sus_neigh_sigma "
                      << n_sigmas * sus_neigh_sigma << std::endl;

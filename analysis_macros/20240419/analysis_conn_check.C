...
int analysis_conn_check(){
    ...
    z_alpha = 0.5;
    for(int ch = 0; ch<128; ch++){
      if (ch%2 ==0 && (ch_hits[ch] < z_alpha*ave_even)){
        broken_channels.push_back(ch);
      }
      else if (ch%2 !=0 && (ch_hits[ch] < z_alpha*ave_odd)){
        broken_channels.push_back(ch);
      }
      else continue;
    }

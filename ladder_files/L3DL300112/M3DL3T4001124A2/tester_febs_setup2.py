...
def reading_lv(pol_usr = 'N'):
    ...
    if (pol == 'N' or pol == '0'):
        ...
        os.system("ssh cbm@cbmflib01 ./mpod_lab2021/lab21_mpod02_u901 voltage | grep -Eo '[0-9]+([.][0-9]+)' > lv_iv.txt")
        os.system("ssh cbm@cbmflib01 ./mpod_lab2021/lab21_mpod02_u900 current | grep -Eo '[0-9]+([.][0-9]+)' >> lv_iv.txt")
    elif(pol == 'P' or pol == '1'):
        print("Reading LV for P-side (holes polarity):")
        #Reading Voltage and current values and writting them in a file                                                               
        os.system("ssh cbm@cbmflib01 ./mpod_lab2021/lab21_mpod02_u903 voltage | grep -Eo '[0-9]+([.][0-9]+)' > lv_iv.txt")
        os.system("ssh cbm@cbmflib01 ./mpod_lab2021/lab21_mpod02_u902 current | grep -Eo '[0-9]+([.][0-9]+)' >> lv_iv.txt")
    else:
...
def powerOn_lv(pol_usr = 'N'):
    ...
    if (pol == 'N' or pol == '0'):
        ...
        #Reading Voltage and current values and writting them in a file                                                               
        os.system("ssh cbm@cbmflib01 ./mpod_lab2021/lab21_mpod02_u901 on")
        os.system("ssh cbm@cbmflib01 ./mpod_lab2021/lab21_mpod02_u900 on")
        time.sleep(10)
        reading_lv(pol)
    elif(pol == 'P' or pol == '1'):
        print("Turning ON LV for P-side (holes polarity):")
        #Reading Voltage and current values and writting them in a file                                                               
        os.system("ssh cbm@cbmflib01 ./mpod_lab2021/lab21_mpod02_u903 on")
        os.system("ssh cbm@cbmflib01 ./mpod_lab2021/lab21_mpod02_u902 on")
        time.sleep(10)
        reading_lv(pol)
    else:
        log.error("Please, indicate a polarity for reading the corresponding LV potentials, in the following way:")
...
def check_moduleId(module_str):
    ...
    if (len(module_sn) == 0):
        log.error("MODULE NAME should not be left empty")
        sys.exit()
        ...
        if (module_id[-2:-1] == 'A' or module_id[-2:-1] == 'B'):
            ...
            log.error("MODULE ID must contain the FEB type (A/B) in the second-to-last position")
            return ['na']
    else:
...
def read_VDDM_TEMP_FEB(feb_sn, smx_l_side, pol):
    ...
    for smx in smx_l_side:
        ...
        #info = "%s\t\t%s\t\t%d\t\t\t%d\t%d\t\t\t%d\t%d", feb_sn, pol_str, smx.address, asic_vddm_src[1], asic_vddm, asic_temp_src[1], asic_temp
        info = "{} \t\t {} \t\t {} \t\t\t {} \t {:.1f} \t\t\t {:.1f} \t {:.1f}".format(feb_sn, pol_str, smx.address, asic_vddm_src[1], asic_vddm, asic_temp_src[0], asic_temp)
        write_data_file(module_dir, module_sn_tmp,info)
        log.info(info)
...
def scan_VrefP_N_Thr2glb(smx_l_side, pol, cal_asic_list, npulses = 100, test_ch = 64, amp_cal_min = 30, amp_cal_max = 247, amp_cal_fast = 30, vref_t = 118):
    smx_cnt = 0
    ...
    for smx in smx_l_side:
        asic_hw_addr = smx.address
        if (asic_hw_addr in cal_asic_list):
            header_line  = "--> SCANNING VREF_P,N & THR2_GLB for ASIC with HW address {} and polarity {}".format(smx.read(192,22), pol_str)
            log.info(header_line)
...
def print_cal_settings(pol, cal_set_side, cal_asic_list, vref_t_calib = 118):
    ...
    log.info(info)
    header_line = "VRef_P \t VRef_N \t VRef_T \t Thr2_glb"
    log.info(header_line)
    smx_cnt = 0
...
def writing_cal_settings(smx_l_side, pol, cal_set_side, cal_asic_list, vref_t_calib = 118):
    vref_n_arr = []
    vref_p_arr = []
    thr2_glb_arr = []
    ...
    log.info(info)
    write_data_file(module_dir, module_sn_tmp, info)
    info = "HW_ADDR: \t VRef_P: \t VRef_N: \t VRef_T: \t VRef_T_range \t Thr2_glb:"
    write_data_file(module_dir, module_sn_tmp, info)
    smx_cnt = 0
    for smx in smx_l_side:
        asic_hw_addr = smx.address
        if (asic_hw_addr in cal_asic_list):
            ...
            # -----------------------------------------------------------------------
            info = "{}\t\t {}\t\t {}\t\t {}\t\t {}\t\t {}".format(asic_hw_addr, smx.read(130,9)&0xff, smx.read(130,8)&0xff, smx.read(130,18)&0xff, vreft_range, smx.read(130,7)&0xff)
            log.info(info)
            write_data_file(module_dir, module_sn_tmp, info)
            smx_cnt +=1
        else:
            info = "--> NO CALIB SETTINGS FOR ASIC with HW ADDRESS {} in {}".format(asic_hw_addr, pol_str)
            log.info(info)
            pass
                
            
# To check calibration function
def calib_FEB(smx_l_side, trim_dir, pol, cal_asic_list, npulses = 40, amp_cal_min = 30, amp_cal_max = 247, amp_cal_fast = 30, much_mode_on = 0):
    ...
    for smx in smx_l_side:
        # Building the file_name for each ASIC
        asic_hw_addr = smx.address
        if (asic_hw_addr in cal_asic_list):
...
def set_trim_calib(smx_l_side, trim_dir, pol, cal_asic_list, much_mode_on = 0):
...
def check_trim(smx_l_side, pscan_dir, pol, cal_asic_list, disc_list = [5,10,16,24,30,31], vp_min = 0, vp_max = 255, vp_step = 1, npulses = 100):
...
def connection_check(smx_l_side, conn_check_dir, pol, cal_asic_list, nloops = 5, vref_t = 108 ):
    ...
    for smx in smx_l_side:
        ...
        if (asic_hw_addr in cal_asic_list):
            # Checking ENC and calibration results for an ASIC     
            info = "CONN-CHECK_ASIC_HW_ADDR_{}: {}".format(asic_hw_addr, pol_str)
            log.info(info)
            write_data_file(module_dir, module_sn_tmp, info)
            write_log_file(module_dir, module_sn, info)
            # Elements for the pscan file                                                                                                                                                         
...
test_list_init = ["#power_on_emu", "full_sync", "read_lv_bc"]
...
test_list_check = ["#set_trim_calib", "#check_trim","#conn_check"]
...
if (nfails ==3):
    log.info("Multiple fails on Writing Module ID. It should contain A or B in the second-to-last position. Please check the Module's information")
    sys.exit()
else:
    pass
...
for test_step in test_list:
    if (test_step == "power_on_emu"):
        ...
        cal_set_nside = scan_VrefP_N_Thr2glb(smx_l_nside, 'N', cal_asic_list_nside, npulses, test_ch, amp_cal_min, amp_cal_max, amp_cal_fast, vref_t)
        cal_set_pside = scan_VrefP_N_Thr2glb(smx_l_pside, 'P', cal_asic_list_pside, npulses, test_ch, amp_cal_min, amp_cal_max, amp_cal_fast, vref_t)
        ...
        writing_cal_settings(smx_l_nside, 'N', cal_set_nside, cal_asic_list_nside)
        writing_cal_settings(smx_l_pside, 'P', cal_set_pside, cal_asic_list_pside)
        ...
        calib_FEB(smx_l_nside, trim_dir, 'N', cal_asic_list_nside, npulses, amp_cal_min, amp_cal_max, amp_cal_fast, much_mode_on = 0)
        calib_FEB(smx_l_pside, trim_dir, 'P', cal_asic_list_pside, npulses, amp_cal_min, amp_cal_max, amp_cal_fast, much_mode_on = 0)
        ...
        set_trim_calib(smx_l_nside, trim_dir, 'N', cal_asic_list_nside, much_mode_on = 0)
        set_trim_calib(smx_l_pside, trim_dir, 'P', cal_asic_list_pside, much_mode_on = 0)
        ...
        check_trim(smx_l_nside, pscan_dir, 'N', cal_asic_list_nside, disc_list, vp_min, vp_max, vp_step, npulses)
        check_trim(smx_l_pside, pscan_dir, 'P', cal_asic_list_pside, disc_list, vp_min, vp_max, vp_step, npulses)
        ...
        write_log_file(module_dir, module_sn, info)
        # Function
        connection_check(smx_l_nside, conn_check_dir, 'N', cal_asic_list_nside, nloops, vref_t_low)
        connection_check(smx_l_pside, conn_check_dir, 'P', cal_asic_list_pside, nloops, vref_t_low)

...
# Loop through each ladder directory in the current directory (e.g., L3DL3)
for ladder_dir in L*/; do
  # Check if the ladder directory exists and is a directory
  if [ -d "$ladder_dir" ]; then
    # Loop through each directory starting with M* inside the ladder directory (e.g., M3DL3B0001120B2)
    for dir in "$ladder_dir"M*/; do

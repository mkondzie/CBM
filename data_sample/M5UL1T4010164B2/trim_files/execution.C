...
void Analysis(){
  ...
  int d_size = sizeof(d_list)/sizeof(int); // 31 is the fast discriminator
  int vp_min = 0;
  int vp_max = 255;
  int vp_step = 1;
  ...
  int cut_db_pulses = 120; // we don't want the second s curve at the top because of the overshoot or undershoot
  // injected pulses
  int rebin_histo = 2;       			// Consider here to put the following values: 1 (typical bin), 2 or 4. Used to remove spikes from the data
  // csa charge sensitive amp
  // 8 bits from 0 to 256 injecting pulse heights 
  // ............... oooo00000oooo........................
